...
class AccessGateTests(unittest.TestCase):
    ...
    def test_enforce_private_dm_blocks_pending_before_heavy_work(self):
        with patch.dict(
            os.environ,
            {
                "USER_ACCESS_APPROVAL_REQUIRED": "true",
                "USER_ACCESS_GUEST_REPLY_QUOTA": "0",
                "RESILIENCE_RUNTIME_DIR": self.tmp.name,
            },
            clear=False,
        ):
            ag.reset_for_tests()
            ag.enqueue_pending("55", username=None, full_name="Guest")
            msg = MagicMock()
            msg.chat.type = "private"
            msg.from_user = MagicMock(username=None, first_name="G", last_name="", is_bot=False, id=55)
            msg.answer = AsyncMock()
            ok = asyncio.run(
                ag.enforce_private_dm_access(
                    msg,
                    user_id="55",
                    bot_user_id=None,
                    is_admin=False,
