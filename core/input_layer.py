...
class InputLayer:
    ...
    async def _process_message_pipeline(
        ...
        if message.chat.type == "private":
            ...
            fu_priv = message.from_user
            bid = self._bot_user_id
            bot_self_uid = (
                (bid is not None and user_id == str(bid))
                or (
                    fu_priv is not None
                    and getattr(fu_priv, "is_bot", False)
                    and user_id == str(fu_priv.id)
                )
            )
            if bot_self_uid:
                # ЛС: автор апдейта — наш бот (часто callback на сообщении бота). Без actor_user_id user_id совпадал
                # с id бота → ложная «заявка». С actor_user_id гейт ниже идёт уже по человеку.
                pass
            elif _agate.is_approval_required() and not self._admin_module.is_admin(user_id):
                fu_priv = message.from_user
                dec = _agate.evaluate_private_user(user_id, is_admin=False)
                if dec == "blocked":
                    await message.answer(_agate.private_blocked_message())
                    return
                _gq = _agate.guest_reply_quota()
                if _gq > 0 and dec in ("pending", "enqueue"):
                    if _agate.guest_replies_used(user_id) >= _gq:
                        await message.answer(_agate.guest_quota_exhausted_message())
                        return
                    if dec == "enqueue":
                        un = ((fu_priv.username if fu_priv else "") or "").strip() or None
                        fn = " ".join(
                            p
                            for p in ((fu_priv.first_name if fu_priv else "") or "", (fu_priv.last_name if fu_priv else "") or "")
                            if p
                        ).strip()
                        is_new = _agate.enqueue_pending(user_id, username=un, full_name=fn or "")
                        if is_new:
                            await self._notify_admins_new_access_request(user_id, un, fn)
                elif dec == "pending":
                    await message.answer(_agate.private_pending_message())
                    return
                elif dec == "enqueue":
                    un = ((fu_priv.username if fu_priv else "") or "").strip() or None
                    fn = " ".join(
            ...
            try:
                ...
                if has_pending(user_id, chat_id):
                    if should_cancel_bug_report_pending(_payload_str):
                        ...
                        logger.info(
                            "[bug_report] pending cancelled: not a bug description (%s)",
                            _payload_str[:80],
                        )
                    else:
