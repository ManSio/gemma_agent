...
def _mem0_key_fingerprint(norm: str) -> Dict[str, Any]:
    """Безопасно для чата: длина, короткий хеш, префикс/хвост (как в логах Mem0)."""
    if not norm:
    ...
    return {
        ...
        "key_len": len(norm),
        "key_sha256_12": h[:12],
        "key_prefix_4": norm[:4] if len(norm) >= 4 else norm,
        "key_tail_4": norm[-4:] if len(norm) >= 4 else norm,
    }
...
def mem0_operator_diagnostics() -> Dict[str, Any]:
    ...
    return {
        ...
        "hints": hints,
        "fix": "Сверьте отпечаток key_sha256_12 после смены ключа; /admin_connectivity — живой запрос к API.",
    }
            ...
            async with session.post(url, headers=self._headers(api_key), json=payload) as resp:
                ...
                if resp.status >= 400:
                    ...
                    if resp.status == 401:
                        ...
                        logger.warning(
                            "Mem0 HTTP 401 %s key_role=%s host=%s key_len=%s key_tail=%s scheme=%s body=%s",
                            path,
                            role,
                            root,
                            len(k),
                            k[-4:] if len(k) >= 4 else k,
                            (os.getenv("MEM0_AUTH_SCHEME") or "token").strip().lower() or "token",
                            text[:400],
                        ...
                        logger.warning("Mem0 HTTP %s %s: %s", resp.status, path, text[:500])
                    return None
                if not text:
                    return {}
                try:
                ...
                if r.status_code == 401:
                    ...
                    if "/memories/add/" in path and role == "mirror":
                        ...
                        record_error_event(
                            ...
                            extra={"path": path, "host": root, "key_len": len(k), "detail": rb[:300]},
                        )
                else:
                    logger.warning("Mem0 HTTP %s %s: %s", r.status_code, path, (r.text or "")[:500])
                return None
