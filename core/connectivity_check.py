...
async def log_mem0_startup_status(mem0_memory: Mem0MemoryModule, log: logging.Logger) -> None:
    ...
    if not mem0_memory._cloud:
        return
    _mp = await check_mem0_platform(mem0_memory._api_key, api_url=mem0_memory._base, role="primary")
    record_external_service_check(_mp, source="startup")
    if _mp.get("ok"):
        log.info("%s", _mp.get("user_message"), extra={"gemma_event": "mem0_primary_ok"})
    else:
        log.warning(
            "%s — долговременная память в облаке может не работать; см. /admin_connectivity",
            _mp.get("user_message"),
            extra={"gemma_event": "mem0_primary_check_failed"},
        )
    if mem0_memory._mirror_key:
        _mm = await check_mem0_platform(
        ...
        if _mm.get("ok"):
            log.info("%s", _mm.get("user_message"), extra={"gemma_event": "mem0_mirror_ok"})
        else:
            log.warning(
                "%s — проверь MEM0_MIRROR_*; см. /admin_connectivity",
                _mm.get("user_message"),
