...
def _reason_tag(reason: str) -> str:
    low = (reason or "").lower()
    if "anomaly escalation" in low:
        return "anomaly_escalation"
    if "critical degradation" in low:
        return "critical_degradation"
    if "stable health" in low or "maintenance cycles" in low:
...
def _write_json(path: Path, data: Dict[str, Any]) -> None:
    ...
    for key, val in data.items():
        if key == "reason":
            safe["reason_tag"] = _reason_tag(str(val or ""))
            safe["reason_len"] = len(str(val or ""))
        elif key == "hint":
            safe["hint_len"] = len(str(val or ""))
        else:
            safe[key] = val
...
class ResilienceController:
    ...
    def enter_safe_mode(self, reason: str, *, level: str = "safe") -> None:
        ...
        logger.warning("Resilience: SAFE MODE — reason_len=%s (%s)", len(reason or ""), level)
    ...
    def exit_safe_mode(self, reason: str) -> None:
        ...
        logger.info("Resilience: safe mode cleared — reason_len=%s", len(reason or ""))
        try:
    ...
    def request_container_restart(self, reason: str) -> None:
        ...
        if not already:
            ...
            logger.error("Resilience: RESTART REQUESTED — reason_len=%s", len(reason or ""))
        else:
            logger.info("Resilience: restart flag already set, updated reason only — reason_len=%s", len(reason or ""))
    ...
    def tick(self, orchestrator: Any, *, maintenance_ran: bool) -> Dict[str, Any]:
        ...
        if self.is_safe_mode():
            st = _read_json(_safe_mode_path())
            if str(st.get("reason_tag") or "") == "anomaly_escalation":
    ...
    def post_boot_recovery(self, orchestrator: Any) -> Dict[str, Any]:
        ...
        if not isinstance(ev, dict):
            ...
            return result
        if ev.get("error"):
            logger.warning("post_boot_recovery: evaluate failed, safe mode unchanged (error_len=%s)", len(str(ev.get("error") or "")))
            return result
