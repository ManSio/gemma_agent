    ...
    async def generate(
        ...
        **kwargs
        ...
        if len(messages) >= 2:
            ...
            if _user_head:
                logger.info("messages[1] head: %s", _user_head)
        ...
        if _telemetry.get("kv_debug_payload") or logger.isEnabledFor(logging.DEBUG):
            _payload_snapshot = {
                ...
                "system_chars": len(str(payload.get("messages", [{}])[0].get("content", ""))) if payload.get("messages") else 0,
                "provider": payload.get("provider"),
                "session_id": _session_id or "",
            }
            logger.debug("openrouter payload snapshot: %s", _payload_snapshot)
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            "llm_cache_telemetry session=%s model=%s upstream=%s prompt_tok=%s cached_tok=%s cache_write=%s reasoning=%s latency_ms=%s",
                            _session_id or "-",
                            model_name,
                            upstream or "?",
