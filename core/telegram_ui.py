...
def format_operator_panel_html(snap: Dict[str, Any]) -> str:
    ...
    if mem0:
        ...
        if pk.get("configured"):
            m0_lines.append(
                f"• primary отпечаток: len=<b>{esc(pk.get('key_len'))}</b> "
                f"sha12=<code>{esc(pk.get('key_sha256_12'))}</code> "
                f"{esc(pk.get('key_prefix_4'))}…{esc(pk.get('key_tail_4'))}"
            )
        ...
        if mk.get("configured"):
            ...
            m0_lines.append(
                f"• mirror отпечаток: len=<b>{esc(mk.get('key_len'))}</b> "
                f"sha12=<code>{esc(mk.get('key_sha256_12'))}</code> "
                f"{esc(mk.get('key_prefix_4'))}…{esc(mk.get('key_tail_4'))}"
            )
