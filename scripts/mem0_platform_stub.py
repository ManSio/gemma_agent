...
Переменные окружения:
  ...
  ENCRYPTION_KEY / MEM0_ENCRYPTION_KEY — Fernet-шифрование файла на диске (plain JSON мигрирует при старте).
...
from core.encrypted_json_store import migrate_plain_to_encrypted, read_encrypted_json, write_encrypted_json
...
def _load_store() -> None:
    ...
    if not path.is_file():
        _store = {}
        return
    migrate_plain_to_encrypted(path)
...
def _persist_store() -> None:
    ...
    path = _data_file
    path.parent.mkdir(parents=True, exist_ok=True)
    if not write_encrypted_json(path, _store):
        raise OSError(f"failed to persist mem0 stub store: {path}")
