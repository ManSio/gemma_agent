...
def audit_host(root: Path, *, host_label: str, days: int) -> Dict[str, Any]:
    ...
    err_top = Counter(
        f"{e.get('component','')}:{str(e.get('kind') or e.get('message',''))[:48]}"
        for e in errs
    )
    ...
    return {
        ...
        "turns": {
            ...
            "samples_incomplete": [
                {
                    ...
                    "user": str(t.get("user_excerpt") or "")[:100],
                    "assistant": str(t.get("assistant_excerpt") or "")[:160],
                }
            ...
            "samples_long_short": [
                {
                    "ts": t.get("ts"),
                    "user": str(t.get("user_excerpt") or "")[:80],
                    "assistant": str(t.get("assistant_excerpt") or "")[:120],
                }
    ...
    for host in report.get("hosts", []):
        ...
        if t.get("samples_incomplete"):
            ...
            for s in t["samples_incomplete"][:4]:
                lines.append(f"- `{s.get('ts')}` {s.get('profile')}: Q={s.get('user')[:60]}…")
                lines.append(f"  A: {s.get('assistant')[:120]}…")
            lines.append("")
